...
После того как в проекте OnlyMele получилось добавлять теги к постам 
и делать сортировку вернулся сюда - попробую оживить этот проект...

17 октября 2021

Вчера сделал ошибку. Решил быстро поменять модель, чтобы иметь возможность 
вносить незарегистрированного владельца поста вместо автора поста. 
Кроме этого хотел сам ставить дату, когда вышел оригинальное видео.
Поскольку миграция не прошла и выходит ошибка - таблицы
отсутствуют, нужно или откатить назад, или полностью сменить базу.
Сначала пробую откатить... 
После внесения изменений в файлах сайт заработал вроде нормально, 
но какие-то изменения в модели уже не проходят - возникает ошибка при миграции. 

ДУМАЛ ПОЛНОСТЬЮ ЗАМЕНИТЬ БАЗУ КАК ЭТО ДЕЛАЛ 'ZANDER', НО НЕ РЕШИЛСЯ! 
БОЮСЬ ПОТЕРЯТЬ РАБОТОСПОСОБНОСТЬ САЙТА - так как хочу его использовать для 
накопления материала и опыта работы с включением нового метериала!

РЕШИЛ ПОЙТИ ПО КНИГЕ 'MELE' С ОГЛЯДКОЙ НА ДАННЫЙ 'ANYBLOGTWO' И ПРИ 
СЛУЧАЕ ПРИМЕНИТЬ ГОТОВЫЙ ШАБЛОН!

16 октября 2021
Снимок (commit): "Боковая панель. Новый Счётчик Просмотров и Комментариев" 

Увидел, что счётчики не работают в боковой панели. Внёс изменения по аналогии
тем, что сделал у самого поста.

15 октября 2021
Снимок (commit): "Новый Счётчик Просмотров" 

Задействованы models.py & views.py (использованы индийские видео)

Сделал подсчёт количества просмотров с использованием счётчика вызова
функции Post, где считается количество вызовов этой функции, а также 
функции time.sleep() из Time module. Причём, каждый вызов страницы поста 
из другой страницы сайта сразу добавляет просмотр, ну а просто обновление 
страницы, даёт прибавку к счётчику только после задержки в несколько 
секунд (5) - окончания которых пользователь может не дождаться, если 
это былo ошибочное обновление. Таким образом, просмотр считается если 
это был действительно просмотр на который прользователь затратил какое-то 
время, даже если просмотр сделан одним и тем же пользователем. 

Есть возможность считать IP адрес, но там нужно чтобы пользователь был 
зарегистрирован (тоже один из индийских блогеров - B Learning Club).
